...
export default function Filmstrip() {
  ...
  useEffect(() => {
    ...
    saveT.current = setTimeout(() => {
      try {
        localStorage.setItem(STORAGE_KEY, JSON.stringify({ films, round }));
      } catch (e) {
        setNote("Couldn't save — browser storage is full or blocked. Export a backup from the Pool tab.");
      }
    }, 500);
  }, [films, round]);
  ...
  const exportBackup = () => {
    const blob = new Blob([JSON.stringify({ films, round }, null, 0)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    const d = new Date().toISOString().slice(0, 10);
    a.href = url; a.download = `filmstrip-backup-${d}.json`;
  ...
  const restoreBackup = (e) => {
    ...
    reader.onload = () => {
      try {
        ...
        if (!Array.isArray(s.films)) throw new Error("shape");
        setUndo({ films, round });
        setFilms(s.films); setRound(s.round || 0); setStrip([]);
        setNote(`Restored ${s.films.length} films from backup.`);
      } catch {
        setNote("That doesn't look like a Filmstrip backup.");
      }
    };
    ...
    <div className="fs" style={{ maxWidth: 520, margin: "0 auto", paddingBottom: 40 }}>
      ...
      {tab === "pool" && (
        <div style={{ padding: 16 }}>
          ...
          <Label style={{ margin: "28px 0 8px" }}>Backup</Label>
          <div style={{ fontSize: 13, color: C.mint, marginBottom: 10, lineHeight: 1.5 }}>
            Everything lives in this browser only. Safari clears site storage now and then, so export a file
            once you’ve put real ranking hours in.
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button className="cta" onClick={exportBackup}
              style={{ background: "transparent", border: `1px solid ${C.amber}`, color: C.amber, fontSize: 14 }}>
              Export
            </button>
            <label className="cta" style={{ textAlign: "center", background: "transparent",
              border: `1px solid ${C.raise}`, color: C.bone, fontSize: 14, cursor: "pointer" }}>
              Restore
